...
\author{
	\href{https://orcid.org/0000-0001-5644-1575}{\includegraphics[scale=0.06]{orcid.png}\hspace{1mm}Amey Thakur} \\
...
\begin{enumerate}[leftmargin=*, topsep=2pt, itemsep=2pt]
  ...
  \item Sub-frame localisation of the event by interpolating the zero between
        ...
        the mean absolute error falling from $33.4$\,ms to $6.7$\,ms.
...
exactly. Related geometric primitives over point sets, including the signed-area
and containment tests we use for the two-hand window, are treated in an
interactive setting by \citet{thakur2022quadtree}.
...
quantities such as joint angles and finger extension. The tradeoff between the
two, and hybrids that soften it, are examined by \citet{thakur2021neurofuzzy},
and the learned side rests on the standard apparatus surveyed in
...
\subsection{Temporal localisation from motion}
...
Restyling a subject is well studied as a learned image-to-image problem, in the
adversarial formulation surveyed by \citet{thakur2021gans} and in the particular
settings of white-box cartoonisation \citep{thakur2021cartoonization}, where the
objective is an explicitly decomposed cartoon representation rather than an
opaque mapping, and open-domain sketch-to-photo synthesis
\citep{thakur2021aoda}. Such methods synthesise content absent from the input,
which is exactly what allows them to replace a subject and exactly what makes
them unsuitable under a constraint of zero marginal cost and bounded latency: a
generative model of useful quality is either hosted, and therefore metered, or
too slow for interactive compositing.
...
\begin{enumerate}[leftmargin=*, topsep=2pt, itemsep=1pt]
  ...
  \item the transit from $|s| \leq \tau_{\mathrm{cross}}$ to the opposite face
        completing within $[60, 600]$\,ms, rejecting single-frame landmark
        inversions below and slow reorientations above;
...
absolute localisation error from $33.4$\,ms to $6.7$\,ms and removes a
$30.5$\,ms bias, at the cost of one division and two stored samples.
...
This is the same distinction that arises between the time an event occurs and
the time it is observed in a distributed system \citep{thakur2022clocksync}: the
useful ordering is by cause, and the observation time is an artefact of the
observer.
...
and both triangles are convex; strictness follows from the corollary below.
\end{proof}
...
divisions, against nine multiplications for the two triangles, so correctness is
obtained at a lower price than the failure it replaces.
...
\end{equation}
thresholded through a hyperbolic tangent in the manner of the extended operator
of \citet{winnemoller2012xdog}. The two kernels are given equal weight, which is
the property the threshold depends on: both integrate to unity, so $D_\sigma$
vanishes identically on any region of constant $L$, whatever that constant is.
...
standard deviation $2.3\times10^{-3}$ against a threshold
$\varepsilon = -3.5\times10^{-3}$. The operator therefore fires on grain at
roughly two thirds of the contrast it requires of a contour, which is precisely
the field of short marks we observed across flat walls. We read it instead from
a mixture
\begin{equation}
  \label{eq:mixture}
  L = (1-\lambda)\,L_{\text{surface}} + \lambda\,L_{\text{raw}}, \qquad \lambda = 0.16,
\end{equation}
where $L_{\text{surface}}$ is the output of \eqref{eq:bilateral}. The raw share
restores detail the reduced-resolution surface cannot carry, and scales the noise
response to $3.7\times10^{-4}$, which places the threshold at $9.4$ standard
deviations. The choice of $\lambda$ is thus fixed by the ratio the operator must
achieve, not by inspection.
...
from either side, so the displacement is continuous across the boundary and the
map has no fixed point outside the ball. For $\|e_t\| > \delta$ the displacement
...
\begin{figure}[t]
  ...
  distinguishable from a recoloured current one. The live frame reads
  $4.3$\,s; during the composite the recalled frames read $2.5$, $2.8$ and
  $2.9$\,s; afterwards, $5.6$\,s. The requested delay was $2.2$\,s.}
  \label{fig:rewind}
\end{figure}
...
Against a flat field carrying per-channel noise of standard deviation $0.06$, the
mean absolute difference between horizontally adjacent output samples inside the
window fell from $4.6$ to $2.7$ of $255$ with the schedule of
Section~\ref{sec:stylise} in place. The residue is the amplification of
\eqref{eq:slope} acting on what the surface still carries, and is what
\eqref{eq:mixture} and the five-tap band selection are sized against.

\paragraph{Separation of the styles.}
The five media are intended to be nameable from a still. We rendered one frame
through each and measured the mean absolute channel difference over the interior
of the window for all ten pairs. The closest pair differed by $14$ of $255$,
against a floor of $12$ set before the measurement; the widest, ink against
comic, by $106$. The measurement is reported because "they look different" is
otherwise an assertion by the author about their own work.

\paragraph{Behavioural coverage.}
Two suites run against the shipped modules. Eighty-five assertions cover the
...
Forty-one further assertions drive the application through its own document with
...
$2.2$\,s, the live frame read $4.3$\,s and the recalled frames read $2.5$, $2.8$
and $2.9$\,s, with the subject visibly displaced. The buffer therefore returns
genuine earlier frames at approximately the requested delay, the residual being
the $125$\,ms capture granularity at $f_b = 8$\,Hz.
...
Thirty-three scripted assertions covering acquisition, recording, review, export,
output presets, filters, effect rebinding, settings persistence, theme selection,
focus containment and keyboard dismissal passed against both the development and
the deployed build.
...
\begin{table}[t]
  ...
  \begin{tabular}{llrr}
    ...
    \midrule
    \textbf{Total}                          &               & \multicolumn{2}{r}{$95.0\%$ detected, $0$ false positives in $240$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  \centering
  \caption{Localisation error on the $114$ detected flips, before and after the
  interpolation of Section~\ref{sec:interpolation}. The sampling interval is
  $41.7$\,ms.}
...
The detector localises the gesture to a tenth of the interval at which it
observes, which is the practical content of Proposition~\ref{prop:interpolation}:
a zero crossing carries information between samples that a per-frame label does
not. A classifier operating on the same stream cannot report an instant finer
than the frame it labels.
